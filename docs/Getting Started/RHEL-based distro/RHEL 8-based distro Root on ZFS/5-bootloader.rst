...
#. GRUB 2.02 has a bug where ``grub2-probe`` does not support ZFS::

     # +++ /usr/sbin/grub2-probe --device /dev/sdd2 --target=fs
     # /usr/sbin/grub2-probe: error: ../grub-core/kern/fs.c:120:unknown filesystem.

   Force ZFS with::

     sed -i 's|fs="`"${grub_probe}" --device ${device} --target=fs`"|fs=zfs|' /usr/share/grub/grub-mkconfig_lib
...
#. Create GRUB boot directory, in ESP and boot pool::
    ...
    mkdir -p /boot/efi/EFI/almalinux        # EFI GRUB dir
    mkdir -p /boot/efi/EFI/almalinux/grub2  # legacy GRUB dir
    mkdir -p /boot/grub2
...
#. If using legacy booting, install GRUB to every disk::
    ...
    for i in ${DISK[@]}; do
     grub2-install --boot-directory /boot/efi/EFI/almalinux --target=i386-pc $i
    done
...
#. Generate GRUB Menu::
    ...
    cp /boot/efi/EFI/almalinux/grub.cfg /boot/efi/EFI/almalinux/grub2/grub.cfg
    cp /boot/efi/EFI/almalinux/grub.cfg /boot/grub2/grub.cfg
...
#. On reboot, system will relabel files for SE Linux,
   on next reboot, append ``rd.break`` to kernel cmdline.
   ...
   See `here <https://www.redhat.com/sysadmin/recover-root-passwd>`__ for details.
